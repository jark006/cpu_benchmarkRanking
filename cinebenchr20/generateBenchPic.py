...
if coreType == 'single':
    ...
    intel_dict = {'i3': 2, 'i5': 0, 'i7': 1, 'i9': 2, 'E5': 3, 'Xeon':3,'Platinum': 3,
                  'Pentium': 2, 'Core2': 2, 'Core22': 2, 'Celeron': 2, 'Celeron2': 2}  # core2系列太多，为避免堆积，分成两拨
    ...
    intel_dict2 = {'i3': 0, 'i5': 1, 'i7': 2, 'i9': 2,
                   'Core2': 0, 'Pentium': 2, 'Celeron': 1, 'Celeron2': 1, 'Atom': 0}

    intel_dict = {'i3': 2, 'i5': 0, 'i7': 1,  'i9': 2, 'E5': 3,'Xeon': 3,'Platinum': 3,
                  'Core2': 3, 'Core22': 2, 'Pentium': 1, 'Celeron': 0, 'Celeron2': 3,}  # core2系列太多，为避免堆积，分成两拨

    amd_dict = {'EPYC': 0, 'TR': 0, 'R9': 1, 'R7': 2, 'R5': 3, 'R3': 1, 'Bulldozer': 0,
                'APU': 0, 'Phenom': 2, 'Athlon': 3, 'Turion': 3}
    # 修正分数
    fix_score = {
       # 'FX-8370': 1300,
       # 'FX-8320': 1090,
...
if buildType != 'debug':
    for y in range(0, imgHigh):
        for x in range(centerOffset + 8, amdOffset - 8):
            h = (int((360 * y / imgHigh) + 360 - math.fabs(x - (centerOffset + centerWidth / 2)))&0xfff0) % 360
