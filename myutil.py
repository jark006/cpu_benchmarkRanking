...
def save_CPU_DataSet(dataPath: str, cpuInfoDict: dict[str, cpuInfo]):
    deprecateCnt = 0
...
def load_CPU_DataSet(dataPath: str):
    cpuInfoList = []
    with open(dataPath, "r") as f:
        ...
        for i in range(lines):
            ...
            cpuInfoList.append(
                cpuInfo(name, score, scoreMulti, vendor, series, platform)
            )
    return cpuInfoList


def save_GPU_DataSet(dataPath: str, gpuInfoDict: dict[str, gpuInfo]):
    with open(dataPath, "w") as f:
        for gpu in gpuInfoDict.values():
            if not gpu.isDeprecated:
                f.write(
                    f"{gpu.name},{gpu.score},{gpu.vendor},{gpu.series},{gpu.platform}\n"
                )


def load_GPU_DataSet(dataPath: str) -> list[gpuInfo]:
    gpuInfoList = []
    with open(dataPath, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            name, score_str, vendor, series, platform = row
            try:
                score = int(score_str)
            except ValueError:
                continue  # 跳过无法解析为整数的 score

            gpuInfoList.append(gpuInfo(name, score, vendor, series, platform))
...
def HSL2RGB(h, s, l):
    ...
    tg = h
    tb = h - v_1_3
    rgb = [tc + 1.0 if tc < 0 else tc - 1.0 if tc > 1 else tc for tc in (tr, tg, tb)]
    rgb = [
        (
            p + ((q - p) * 6 * tc)
            if tc < v_1_6
